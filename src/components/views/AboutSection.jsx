...
export default function AboutSection({ text }) {
    ...
    return (
        <SectionWrapper id="about" backgroundColor={'#000000'} backgroundUrl={`/${"Code Background Center.png"}`}>
            <SectionHeading>About</SectionHeading>
                <AboutContent>
                    {/* Depending on how your new line is delimited, this functionality will need to be changed. */}
                    {text || dummyText.split("<br />").map((x, i) => i === 0 ? <div key={i}>{x}</div> : <div key={i}><br /><div>{x}</div></div>)}
                </AboutContent>
        </SectionWrapper>
    )
};